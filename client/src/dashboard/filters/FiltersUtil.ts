...
export function getDesiredKeysFromCurrentFilteredKeys(
  ...
  if (currentFilterType == FilterType.SERVICES) {
    if (desiredFilterType == FilterType.CLOUD_PROVIDERS) {
      getCloudProvidersFromServices(currentSelections).forEach((cloudProviderOption) =>
        desiredSelections.add(cloudProviderOption),
      )
    }
    if (desiredFilterType == FilterType.ACCOUNTS) {
      const currentCloudProviders = Array.from(getCloudProvidersFromServices(currentSelections))
      currentCloudProviders.forEach((currentCloudProvider) => {
        ...
        if (isDropdownOptionInDropdownOptions(oldSelections.cloudProviders, currentCloudProvider)) {
  ...
  if (currentFilterType === FilterType.ACCOUNTS) {
    if (desiredFilterType === FilterType.CLOUD_PROVIDERS) {
      getCloudProvidersFromAccounts(currentSelections).forEach((cloudProviderOption) =>
        desiredSelections.add(cloudProviderOption),
      )
    }
    if (desiredFilterType === FilterType.SERVICES) {
      const currentCloudProviders = Array.from(getCloudProvidersFromAccounts(currentSelections))
      currentCloudProviders.forEach((currentCloudProvider) => {
  ...
  return addAllDropDownOption(desiredSelections, desiredFilterType)
}
...
export function getCloudProvidersFromAccounts(accountSelections: DropdownOption[]): Set<DropdownOption> {
...
export function serviceTypesInAccountSelection(accountSelections: DropdownOption[]): DropdownOption[] {
...
export function addAllDropDownOption(currentSelections: Set<DropdownOption>, filterType: FilterType): DropdownOption[] {
...
export function handleSelections(
  ...
  const oldSelectionKeys: string[] = getSelectionKeysOfFilterType(oldSelections, filterType).map(
  ...
  if (!selectionKeys.includes(ALL_KEY) && oldSelectionKeys.includes(ALL_KEY)) {
    ...
    providerOptions =
      filterType == FilterType.CLOUD_PROVIDERS
        ? selections
        : getDesiredKeysFromCurrentFilteredKeys(selections, oldSelections, filterType, FilterType.CLOUD_PROVIDERS)
    accountOptions =
      filterType == FilterType.ACCOUNTS
        ? selections
        : getDesiredKeysFromCurrentFilteredKeys(selections, oldSelections, filterType, FilterType.ACCOUNTS)
    serviceOptions =
      filterType == FilterType.SERVICES
        ? selections
        : getDesiredKeysFromCurrentFilteredKeys(selections, oldSelections, filterType, FilterType.SERVICES)
  }
  return { providerKeys: providerOptions, serviceKeys: serviceOptions, accountKeys: accountOptions }
}

export function getSelectionKeysOfFilterType(
