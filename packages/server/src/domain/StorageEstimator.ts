...
import { CLOUD_PROVIDER_EMISSIONS_FACTORS_METRIC_TON_PER_KWH, CLOUD_CONSTANTS } from './FootprintEstimationConstants'
...
export class StorageEstimator implements IFootprintEstimator {
  ...
  estimate(data: StorageUsage[], region: string, cloudProvider: string): FootprintEstimate[] {
    return data.map((d: StorageUsage) => {
      const estimatedKilowattHours = this.estimateKilowattHours(d.terabyteHours, cloudProvider, region)
  ...
  private estimateKilowattHours(terabyteHours: number, cloudProvider: string, region: string) {
    ...
    return (terabyteHours * this.coefficient * CLOUD_CONSTANTS[cloudProvider].getPUE(region)) / 1000
  }

  private estimateCo2(estimatedKilowattHours: number, region: string, cloudProvider: string) {
    ...
    return estimatedKilowattHours * CLOUD_PROVIDER_EMISSIONS_FACTORS_METRIC_TON_PER_KWH[cloudProvider][region]
  }
}
